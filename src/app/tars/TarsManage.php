...
class TarsManage
{
    ...
    public function getTarsConf(){
	    $root_path = root_path(); //tars 配置文件路径
        $tars_conf = dirname(dirname($root_path)) .DIRECTORY_SEPARATOR.'conf'.DIRECTORY_SEPARATOR.app()->config->get('app.name', 'ThinkPHP').'.config.conf';
    ...
    public function getMasterAlivePid()
    {
        ...
        if( empty($processName) ){
            ...
            $cmd = "ps aux | grep swoole_http_server:\ worker\ process\ for\ ".$processName." | grep -v grep  | awk '{ print $2}'";
            exec($cmd, $ret);
            if( empty($ret) ) return 0;
            unset($ret);
            $cmd = "ps aux | grep swoole_http_server:\ master\ process\ for\ ".$processName. " | grep -v grep  | awk '{ print $2}'";
            exec($cmd, $ret);
            if( empty($ret) ) return 0;
